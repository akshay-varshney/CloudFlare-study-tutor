...
│  useAgentChat() ←── WebSocket ──→ ChatAgent (Durable Object)│
└─────────────────────────────────────────────────────────────┘
                                │
                ┌───────────────┼───────────────┐
                ▼               ▼               ▼
         ┌───────────┐  ┌────────────┐  ┌──────────────┐
         │ Workers AI │  │   SQLite   │  │  Scheduling  │
         │ (Llama 4)  │  │  State &   │  │  (Reminders) │
         │ streamText │  │  Memory    │  │  Spaced Rep  │
         └───────────┘  └────────────┘  └──────────────┘
```
...
- 🎓 **Interactive Teaching** — Feynman-technique explanations on any topic
- 📝 **Quiz Generation** — Auto-graded quizzes with score tracking
- 🃏 **Flashcard System** — Spaced repetition scheduling
- 📊 **Progress Tracking** — Streaks, confidence levels, weak/strong topics
- ⏰ **Study Reminders** — Natural language scheduling
- 💾 **Persistent Memory** — Everything survives restarts via Durable Objects + SQLite
...
- "Show my study stats"
- "Remind me to study algorithms in 30 minutes"

## Tech Stack

Workers AI • Agents SDK • Durable Objects • React 19 • Vite 8 • Tailwind CSS
